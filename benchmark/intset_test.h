...
// to run the benchmark in a variety of ways, can create an ARGS object
//   and add it to Remus's command-line argument tool:
...
// and can report run-time statistics
//         (tricky bc threads on different machines will need to combine statistics)

// metrics used to track events during the execution of an experiment
struct Metrics {
...
template <typename S, typename K> struct IntSetTest {
    ...
    uint64_t node_id_;      // the ID of the compute node where the thread is
    ...
    void collect(CT &ct, rdma_ptr<Metrics> g_metrics) {
        ...
        ct->FetchAndAdd(rdma_ptr<uint64_t>(base + offsetof(Metrics, get_t)), metrics_.get_t); 
        ct->FetchAndAdd(rdma_ptr<uint64_t>(base + offsetof(Metrics, get_f)), metrics_.get_f); 
        ct->FetchAndAdd(rdma_ptr<uint64_t>(base + offsetof(Metrics, ins_t)), metrics_.ins_t); 
        ...
        ct->FetchAndAdd(rdma_ptr<uint64_t>(base + offsetof(Metrics, read_bytes)), ct->metrics_.read.bytes);
        ct->FetchAndAdd(rdma_ptr<uint64_t>(base + offsetof(Metrics, faa_ops)), ct->metrics_.faa);
        ct->FetchAndAdd(rdma_ptr<uint64_t>(base + offsetof(Metrics, cas_ops)), ct->metrics_.cas); 
    }
