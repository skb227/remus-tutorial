...
int main(int argc, char **argv) {
  ...
  if (id >= c0 && id <= cn) {
    ...
    if (id == c0) {
      ...
      // FLUSH THE ROOT POINTER TO THE MEMORY NODE
      // volatile auto _fr = compute_threads[0]->get_root<LazyListSet>();
      // _fr;
    }
    ...
    for (uint64_t i = 0; i < args->uget(remus::CN_THREADS); i++) {
      worker_threads.push_back(std::thread(
          [&](uint64_t i) {
            ...
            std::cout << "past first barrier in worker thread" << std::endl;
            ...
            // --- NEW FIX: SPIN UNTIL ROOT IS VISIBLE ---
            auto set_ptr = ct->get_root<set_t>();
            while ((uintptr_t)set_ptr == 0) {
                set_ptr = ct->get_root<set_t>();
            }
            set_t set_handle(set_ptr); 

            // Make a workload manager for this thread
            test_t workload(set_handle, i, id);
            ct->arrive_control_barrier(total_threads);

            std::cout << "past second barrier in worker thread" << std::endl;

            // Prefill the data structure
            workload.prefill(ct, args);
            ct->arrive_control_barrier(total_threads);

            std::cout << "past third barrier in worker thread" << std::endl; 
            ...
            std::cout << "past fourth barrier in worker thread" << std::endl; 

            // Run the workload
            workload.run(ct, args);
            ct->arrive_control_barrier(total_threads);

            std::cout << "past fifth barrier in worker thread" << std::endl; 
            ...
            std::cout << "everything reclaim deferred" << std::endl; 
            ...
            std::cout << "past sixth barrier in worker thread" << std::endl; 
            ...
            std::cout << "past seventh barrier in worker thread" << std::endl; 
